...
public class BoundedDifferenceCalculator {
    private final List<DoubleValueTracker> tracker = new Vector<>();
    private final Boolean isDefinedBySinglePort;

    public BoundedDifferenceCalculator(final Observable observable, final List<Variable> variables) {
    ...
    public Double getRelativeDifference() {
        if (isDefinedBySinglePort) {
            ...
            return getAbsoluteDifference() / Math.max(Math.max(Math.abs(currentValue), Math.abs(previousValue)), Double.MIN_VALUE);
        } else {
            final List<Double> currentValues = getCurrentPortValues();
    ...
    public Double getNextRelativeDifference() {
        if (isDefinedBySinglePort) {
            final Double currentValue = tracker.get(0).getCurrentValue();
            final Double nextValue = tracker.get(0).getNextValue();
            return getNextAbsoluteDifference() / Math.max(Math.max(Math.abs(currentValue), Math.abs(nextValue)), Double.MIN_VALUE);
        } else {
            final List<Double> nextValues = getNextPortValues();
    ...
    private List<Double> getCurrentPortValues() {
        final List<Double> currentValues = new Vector<>();
    ...
    private List<Double> getNextPortValues() {
        final List<Double> nextValues = new Vector<>();
        for (DoubleValueTracker t : tracker) {
